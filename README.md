...
## Custom Distillation Workflow

This fork also contains a custom SlimHRNet distillation workflow for pseudo-label generation, frame extraction, training, visual testing, and benchmarking. See [README_DISTILL.md](README_DISTILL.md) for the end-to-end commands.
...
```bash
python src/main.py --config-name=inference_blurball detector.model_path=<path to corresponding model> +input_vid=<path to vid> detector.step=1 detector.postprocessor.score_threshold=0.7
```
