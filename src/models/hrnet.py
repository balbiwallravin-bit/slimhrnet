...
class HighResolutionModule(nn.Module):
    def __init__(self, num_branches, blocks, num_blocks, num_inchannels,
                 num_channels, fuse_method, multi_scale_output=True, use_se=False):
    ...
    def _build_block(self, block, inplanes, planes, stride=1, downsample=None):
...
class HRNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.stage2, pre_stage_channels = self._make_stage(
            self.stage2_cfg, num_channels, use_se=self._use_se)
        ...
        self.stage3, pre_stage_channels = self._make_stage(
            self.stage3_cfg, num_channels, use_se=self._use_se)
    ...
    def _make_layer(self, block, inplanes, planes, blocks, stride=1, use_se=False):
        downsample = None
        ...
        for i in range(1, blocks):
            if block is BasicBlock:
                layers.append(block(inplanes, planes, use_se=use_se))
            else:
    ...
    def _make_stage(self, layer_config, num_inchannels,
                    multi_scale_output=True, use_se=False):
